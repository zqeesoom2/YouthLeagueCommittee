   ...
   public function add($data) {
    ...
    public function adminPage($where,$num) {
       if(is_array($where)){
           ...
           return self::field('Id,regist_auth,status,group_id,title,area,recruit_time_start,recruit_time_end,activity_time_start,activity_time_end,recruit_num,service_id')
               ->where('status',4)->whereOr('status',5)->order('id', 'desc')->paginate($num)->each(function($item, $key){
